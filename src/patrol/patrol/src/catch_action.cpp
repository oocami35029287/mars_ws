...
{  
    ...
    dope_sub = nh.subscribe("/det3d_result", 1, &warehouse_action::dope_callback, this);
...
void warehouse_action::dope_callback(walker_msgs::Det3DArray msg)
{
    // cout<<"111111111111"<<endl;
    if(msg.dets_list.size() != 0 && reach){
        dope_list = msg;
        reach = false;
        cout<<msg.dets_list[0].class_name<<endl;
...
void warehouse_action::Position_Manager()
{
    ...
    //gripper
    actionlib::SimpleActionClient<control_msgs::GripperCommandAction> gripper_action("/gripper_controller/gripper_cmd", true);
    gripper_action.waitForServer(ros::Duration(3));
    control_msgs::GripperCommandGoal gripgoal;
    ...
    while(1)
    {
        ...
        if(command =='c'){
            gripgoal.command.position = 0.75;
            gripper_action.sendGoal(gripgoal);
            gripper_action.waitForResult();
            /*if (gripper_action.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
                ROS_INFO("You have reached the goal!\n");
            else
                ROS_INFO("The base failed for some reason\n");*/
        }
        if(command =='o'){
            gripgoal.command.position = 0.03;
            gripper_action.sendGoal(gripgoal);
            gripper_action.waitForResult();
            /*if (gripper_action.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
                ROS_INFO("You have reached the goal!\n");
        ...
        if(command == 'g'){
            ...
            //input what to find 
            reach = true;
            printf("pleace insert the object you want to catch.>>");
            std::cin>>text;
            sleep(1);
            
            //visualization_msgs::MarkerArray snull = [];
            // if(dope_list.empty()){
            //     cout<<"No DOPE object found."<<endl;
            //     goto lab_end;
            // }
            for(walker_msgs::Det3D &itr:dope_list.dets_list){
                //std::string dope_str = itr.text.substr(0,itr.text.find('('));
                //cout<<dope_str<<endl;
                cout<<itr.class_name<<endl;
                if(!strcmp(itr.class_name.substr(0,text.length()).c_str(),text.substr(0,text.length()).c_str())){ 
                    //success compare;
                    object_pose.position.x = itr.x;
                    object_pose.position.y = itr.y;
                    object_pose.position.z = itr.z;
                    cout<<"get!"<<endl;
                    check = true; 
                    continue;
                }
            ...
            if(check){
                ...
                tf.setOrigin(tf::Vector3(object_pose.position.x,object_pose.position.y, object_pose.position.z));
                tf.setRotation(tf::Quaternion(current_pose.orientation.x, current_pose.orientation.y, current_pose.orientation.z, current_pose.orientation.w));
                br.sendTransform(tf::StampedTransform(tf, ros::Time::now(), "camera1_realsense_gazebo", "/tm_target_pose" ));
                
                current_pose.position.x -= tf.getOrigin().getX();
                current_pose.position.y -= tf.getOrigin().getZ();
                current_pose.position.z -= tf.getOrigin().getY();
